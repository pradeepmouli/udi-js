...
import type { FamilyDef, CategoryDef, DeviceDef, DeviceMap } from '../dist/esm/types/Devices/DeviceMap.js';
...
import type { Category } from '../dist/esm/types/Definitions/Global/Categories.js';
import { parseNLSContent } from '../dist/esm/Model/NLS.js';
import { ISY } from '../dist/esm/ISY.js';
import winston from 'winston';
import { setUncaughtExceptionCaptureCallback } from 'process';
import { buildNodeClassDefinitions, NodeClassDefinition } from '../dist/esm/Model/ClassDefinition.js';
import type { NodeDef, NodeDefs } from '../dist/esm/Model/NodeDef.js';
            ...
            if (f.dir) {
                if (f.file) {
                    try {
                        ...
                        var family = p.family as typeof Family;
                        if (data) {
...
function parseNLS(data: string, family: typeof Family = Family.Insteon) {
...
function parseNodeDefs(data: any, family: typeof Family = Family.Insteon) {
    //const filePath = path.join(__dirname, "nls.txt");
...
export function buildDeviceMap() {
    ...
    DeviceMapJSON.forEach((item) => {
        var id = item.id as typeof Family;

        var fam: FamilyDef<typeof Family> = { id: item.id, description: item.description, name: item.name as any, categories: {} };
        ...
        item.categories.forEach((element) => {
            ...
            var catDef: CategoryDef<typeof Family, Category> = { id: element.id, name: element.name, devices: {} };
