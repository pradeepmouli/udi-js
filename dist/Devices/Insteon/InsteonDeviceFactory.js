...
export class InsteonDeviceFactory {
    static buildDeviceMap() {
        ...
        s.forEach((item) => {
            var id = item.id;
            fams.set(id, { id: item.id, description: item.description, name: item.name, categories: new Map() });
            var famDef = fams[id];
            item.categories.forEach((element) => {
                var catDef = { id: element.id, name: element.name, devices: new Map };
    ...
    static getSwitchLightInfo(deviceCode, subAddress) {
        const c = String.fromCharCode(deviceCode);
        let retVal = { name: 'Generic Insteon Relay', class: InsteonRelayDevice };
        switch (c) {
            case String.fromCharCode(0o006):
    ...
    static getClimateControlInfo(deviceCode) {
        ...
        if (retVal?.class === undefined) {
            retVal.class = InsteonThermostatDevice;
        }
        return retVal;
    }
