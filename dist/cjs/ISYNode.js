...
exports.ISYDeviceNode = exports.ISYOldDevice = exports.ISYNode = void 0;
...
class ISYOldDevice extends ISYNode {
    typeCode;
    deviceClass;
    parentAddress;
    category;
    ...
    parseResult(node, device) {
        if (Array.isArray(node.property)) {
            ...
            device.logger(`Property ${ISY_js_1.Controls[node.property.id].label} (${node.property.id}) refreshed to: ${device[node.property.id]} (${device.formatted[node.property.id]})`);
        }
    }
    applyStatus(device, prop) {
        ...
        device.logger(`Property ${ISY_js_1.Controls[prop.id].label} (${prop.id}) refreshed to: ${device[prop.id]} (${device.formatted[prop.id]})`);
    }
    handleControlTrigger(controlName) {
...
exports.ISYOldDevice = ISYOldDevice;
