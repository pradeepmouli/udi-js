...
class ISY extends events_1.EventEmitter {
    ...
    async callISY(url) {
        ...
        try {
            const response = await (0, Utils_1.getAsync)(url, this.restlerOptions);
    ...
    async loadConfig() {
        try {
            ...
            if (this.debugLoggingEnabled) {
                (0, fs_1.writeFile)(this.storagePath + '/ISYConfigDump.json', JSON.stringify(result), this.logger.error);
            }
    ...
    async initialize(initializeCompleted) {
        ...
        try {
            ...
            await this.refreshStatuses().then(() => {
                if (this.elkEnabled) {
                    (0, restler_base_1.get)(`${this.protocol}://${that.address}/rest/elk/get/topology`, options).on('complete', (result, response) => {
                        if (that.checkForFailure(response)) {
                            ...
                            (0, restler_base_1.get)(`${that.protocol}://${that.address}/rest/elk/get/status`, options).on('complete', (result, response) => {
                                if (that.checkForFailure(response)) {
            ...
            switch (stringControl) {
                ...
                default:
                    if (evt.node !== '' && evt.node !== undefined && evt.node !== null) {
                        ...
                        this.logger.info(`Unhandled ${EventType_1.EventType[Number(stringControl)]} Event: ${JSON.stringify(evt)}`);
                    }
                    break;
            }
        });
    ...
    initializeWebSocket() {
        ...
        that.logger.info(`Connecting to: ${this.wsprotocol}://${this.address}/rest/subscribe`);
