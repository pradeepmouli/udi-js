...
const EnumDefinition_js_1 = require("./EnumDefinition.js");
    ...
    for (const id in map) {
        ...
        for (const id2 of Object.keys(map).filter((k) => id !== k)) {
            if (map[id2].implements.includes(id) || node.implements.includes(id2) || node.equivalentTo.includes(id2)) {
                ...
                const node2 = map[id2];
                const d = (0, microdiff_1.default)(JSON.parse(JSON.stringify(node2)), JSON.parse(JSON.stringify(node)));
                let ext = false;
                let equiv = false;
...
class NodeClassDefinition {
    ...
    get name() {
        return `${(0, moderndash_1.pascalCase)(this.label ?? this.id ?? 'Unknown')}`;
    }
    ...
    applyEditorDefs() {
        for (const driver of Object.values(this.drivers)) {
            if (driver.editorId) {
                let d = EditorDef_js_1.EditorDef.get(this.family, driver.editorId);
                if (d)
        ...
        for (const cmd of Object.values(this.commands)) {
            if (cmd.editorId) {
                let d = EditorDef_js_1.EditorDef.get(this.family, cmd.editorId);
                if (d)
                    cmd.applyEditorDef(d);
            }
            for (const p of Object.values(cmd.parameters ?? {})) {
                if (p.editorId) {
                    let d = EditorDef_js_1.EditorDef.get(this.family, p.editorId);
                    if (d)
                        p.applyEditorDef(d);
                }
            }
    ...
    applyIndexDefs() {
        let NLSIndexMap = NLS_js_1.IndexDef.Map;
    ...
    applyNLS() {
        let NLSRecordMap = NLS_js_1.NLS.Map;
    ...
    toJSON() {
        return {
            ...
            name: this.name,
            dynamic: this.family in [Families_js_1.Family.ZWave, Families_js_1.Family.ZigBee],
            ...
            for (const rangeDef of (0, Utils_js_1.toArray)(e.range)) {
                if ('subset' in rangeDef) {
                    ...
                    d[rangeDef.uom] = {
                        ...
                        returnType: rangeDef.nls ? EnumDefinition_js_1.EnumDefinition.get(this.classDef.family, rangeDef.nls, rangeDef.uom, this.classDef.id)?.name : undefined
                    };
                }
            }
            this.dataType = d;
...
class CommandDefinition extends NodeMemberDefinition {
    ...
    applyNLSRecord(nls) {
        ...
        else if (nls.type === NLS_js_1.NLSRecordType.CommandParameter || nls.type === NLS_js_1.NLSRecordType.CommandParameterNLS) {
            if (this.parameters && this.parameters[nls.control]) {
                this.parameters[nls.control].applyNLSRecord(nls);
            }
        }
    ...
    function load(path) {
        for (const file of new Set(fs.readdirSync(path + '/generated').concat(fs.readdirSync(path + '/overrides')))) {
            ...
            if (fs.existsSync(`${path}/overrides/${fam}.json`)) {
                nodeClassDefs = (0, moderndash_1.merge)(nodeClassDefs, JSON.parse(fs.readFileSync(`${path}/overrides/${fam}.json`, 'utf8')));
            }
...
})(NodeClassDefinition || (exports.NodeClassDefinition = NodeClassDefinition = {}));
