...
import { Devices as DevicesNS, Insteon } from '../../Devices/index.js';
import { CommandsOf, DriversOf, ISYNode } from '../../ISYNode.js';
import { ClusterType } from '../../Model/ClusterType.js';
import type { ExtractKeys, PathsWithLimit, PickOfType, RelaxTypes, Remove, StringKeys } from '../../Utils.js';
...
export type FamilyToClusterMap<T extends keyof Devices> = { Family: T; } & {
...
export function add<const F extends keyof Devices, const Y extends keyof Devices[F], const D extends MutableEndpoint>(
...
type SupportedDevices = keyof Devices;
export class MappingRegistry {
	...
	public static add<T extends SupportedDevices, D extends keyof Devices[T], M extends MutableEndpoint>(mapping: { [x in D]?: DeviceToClusterMap<Devices[T][x], M> }): typeof MappingRegistry {
		MappingRegistry.register<T>(mapping as unknown as FamilyToClusterMap<T>);
		return MappingRegistry;
	}
	//@ts-ignore
	public static register<const T extends keyof Devices>(map: FamilyToClusterMap<T>, family?: T): void {
		if ('Family' in map) {
